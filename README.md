...
- 사용자 나이 / 성별 / 선호 카테고리 / 선호 계절 / 현재 계절 기반 맞춤 추천
- 여행지 점수 계산(카테고리 선호도, 계절 적합도, 연령대 가중치)을 통한 지역 선정
- OpenAI GPT를 활용한 **1일 여행 일정 자동 생성**
- 동일 지역 여행지 묶음을 기반으로 한 일정 구성
- 추천 결과 팝업 UI 제공 및 추천 이력 저장 기능
